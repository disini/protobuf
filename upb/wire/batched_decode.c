...
static void upb_BranchlessStoreUint(void* to, const char* from, size_t size) {
...
static const char* _upb_BatchDecoder_DecodeToSubMessage(
    upb_BatchDecoder* d, const char* ptr, upb_Message* msg,
    const upb_MiniTableSub* subs, const upb_MiniTableField* f, int size) {

  void* mem = UPB_PTR_AT(msg, f->UPB_PRIVATE(offset), void);
  if (upb_MiniTableField_IsString(f)) {
    if (f->UPB_PRIVATE(descriptortype) == kUpb_FieldType_String) {
    ...
    if (UPB_UNLIKELY(d->end - ptr < 8)) {
      ptr = upb_BatchDecoder_ReadUint(d, ptr, mem, size);
    } else {
      // Branchless fast path.
      ptr = upb_BatchDecoder_ReadUint(d, ptr, &data, 8);
      data &= (1ULL << (field_size * 8)) - 1;
    }
    ...
    upb_BranchlessStoreUint(mem, ptr, field_size);
  }

  // Set presence if necessary.
  // We do this after the field is set so that the hasbit is set only if the
  // field is actually set (and not if we bail for invalid UTF-8 or enum).
  if (UPB_PRIVATE(_upb_MiniTableField_HasHasbit)(f)) {
    ...
    uint32_t* oneof_case = UPB_PRIVATE(_upb_Message_OneofCasePtr)(msg, f);
    *oneof_case = f->UPB_PRIVATE(number);
  }
...
const char* _upb_BatchDecoder_DecodeKnownField(
    ...
    const upb_MiniTable* layout, const upb_MiniTableField* field, int size) {
  ...
  switch (mode & kUpb_FieldMode_Mask) {
    ...
    case kUpb_FieldMode_Scalar:
      return _upb_BatchDecoder_DecodeToSubMessage(d, data, msg, subs, field,
                                                  size);
    default:
      UPB_UNREACHABLE();
  }
...
static const char* upb_BatchDecoder_DecodeBatch(upb_BatchDecoder* d,
  ...
  for (int i = 0; i < batch_size; i++) {
    ...
    const upb_MiniTableField* f =
        _upb_BatchDecoder_FindField(d, m, field_number, &last_field_index);

    if (f->UPB_PRIVATE(descriptortype) == kUpb_FakeFieldType_FieldNotFound) {
      // We don't support unknown fields yet.
      _upb_BatchDecoder_ErrorJmp(d, kUpb_DecodeStatus_Malformed);
    }

    data = _upb_BatchDecoder_DecodeKnownField(d, data, msg, m, f, field_size);
  }
...
static const char* upb_BatchDecoder_DecodeSubMessages(upb_BatchDecoder* d,
  ...
  if (upb_MiniTableField_IsArray(f)) {
    ...
    elems += arr->UPB_PRIVATE(size);
    for (int i = 0; i < batch_size; i++) {
      elems[i] = upb_Message_New(subl, &d->arena);
